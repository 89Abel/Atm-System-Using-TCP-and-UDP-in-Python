...
db_local_bank = client_local_bank['Bank_A']
collection_name = db_local_bank['Users']
...
user_data = [
    {"username": "Abel", "password": "12345", "balance": "10,000 Birr"},
    {"username": "Sami", "password": "67890", "balance": "15,000 Birr"},
    {"username": "Tina", "password": "01234", "balance": "20,000 Birr"}
]
...
def handle_udp():
    ...
    print("Bank A Server is listening for UDP connections...")
    ...
    while True: 
        ...
        user_data_local = db_local_bank[collection_name].find_one({"username": username, "password": hashed_password})

        if user_data_local:
            balance = user_data_local.get("balance", "N/A")
            response = f"Local Bank A - Balance: {balance}"
            ...
            try:
                ...
                tcp_client_socket.connect(('localhost', 5000))
...
def handle_tcp():
    # MongoDB connection for the local bank
    client_local_bank = MongoClient('mongodb://localhost:27017/')
    db_other_bank = client_local_bank['Bank_A']

    # Create a TCP connection to handle request from the other bank
    tcp_server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    tcp_server_socket.bind(('localhost', 6000))
    tcp_server_socket.listen(5)

    print("Other Bank A Server is listening for TCP connections...")
    while True:
        ...
        if user_data_other_bank:
            ...
            balance_response = f"Balance from Other Bank A: {balance}"
        else:
            balance_response = "Invalid username or password from Bank A"
